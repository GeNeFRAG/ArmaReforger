...
test.describe('Fire for Effect', () => {
  ...
  let calcPage;
   ...
   test.beforeEach(async ({ browser }) => {
    ...
    // Wait for debounced validateCoordinateRange (500ms) to settle so it
    // doesn't race with FFE generation and clear the output mid-render.
  ...
  test('should generate lateral sheaf pattern', async () => {
    ...
    // Wait for FFE generation to complete using Playwright's auto-retrying assertion.
    // "Fire for Effect Mission" is the <h2> heading rendered by generateFFEDisplayHTML,
    // only present after FFE runs (unlike the always-visible "Enable Fire for Effect
    // Pattern" label inside #ffeContainer).
    const output = page.locator('#output');
    await expect(output).toContainText('Fire for Effect Mission', { timeout: 10000 });
  ...
  test('should display multiple firing solutions for FFE', async () => {
    ...
    // Check that rounds are generated - the output shows "X of 5 rounds (in range)"
  ...
  test('should return to base solution when button clicked', async () => {
    ...
    // Wait for base solution to be restored - "Fire for Effect Mission" heading is gone
    await expect(output).not.toContainText('Fire for Effect Mission', { timeout: 10000 });
